...
data Metadata =
  Metadata
    ...
    -- ^ The time the file was created. This time can be after the publish date,
    -- in which case it indicates that I wrote and published the article's
    -- content elsewhere before I migrated it to my personal site.
    , metaPublished :: Maybe (Time.UTCTime, String)
    -- ^ The time the file was published to the internet. This type of time is
    -- only used for articles like blog posts. It can be older than the creation
    -- time of the file, if the article was migrated from a different site.
    , metaModified :: Maybe (Time.UTCTime, String)
    -- ^ The time the file was modified. This indicates the time the content of
    -- the document was last edited. This time is not updated when the metadata
    -- of the document is edited or when the document's content does not
    -- meaningfully change (such as when link urls are updated to point to the
    -- same documents that have been moved).
    , metaTags :: [Text]
...
extractMetadata metaSourcePath (Pandoc.Pandoc (Pandoc.Meta meta) _) = do

  let baseName = FilePath.takeBaseName metaSourcePath
